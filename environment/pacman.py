...
def loadAgent(pacman, nographics):
    # os.environ["Agents"] = os.path.split(os.path.realpath(__file__))[0] + "\\Agents"
    ...
    pythonPathDirs.append(os.path.split(os.path.realpath(__file__))[0] + "\\Agents")
...
def replayGame(layout, actions, display):
    from environment import pacmanAgents
    from environment import ghostAgents
...
def runGames(trial, layout, horizon, pacman, ghosts, display, numGames, record, numTraining=0, catchExceptions=False,
    ...
    for i in range(numGames):
        ...
        strategy_utility_sequences.append(strategy_utility_sequence)
        if (i + 1) % 10 == 0:
            print("save!")
            result = {
    ...
    return games, state_sequences, action_sequences, files, reward_sequences
...
if __name__ == '__main__':
    ...
    trial = 1
    # with open("./sequence_data/smallGrid.pkl", "rb") as file:
